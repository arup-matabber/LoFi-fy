...
export const getInstrument = (instrument: Instrument, context?: Tone.BaseContext) => {
  // Temporarily set the context if provided
  const originalContext = context ? Tone.getContext() : null;
  if (context) {
...
export const getInstrumentFilters = (instrument: Instrument, context?: Tone.BaseContext) => {
  // Temporarily set the context if provided
  const originalContext = context ? Tone.getContext() : null;
  if (context) {
    Tone.setContext(context);
